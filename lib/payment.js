...
var URLS = {
  ...
  SEND_REDPACK: 'https://api.mch.weixin.qq.com/mmpaymkttransfers/sendredpack',//发红包
  TRANSFERS: 'https://api.mch.weixin.qq.com/mmpaymkttransfers/promotion/transfers'//企业付款
};
...
Payment.prototype._signedQuery = function(url, params, options, callback) {
  ...
  if(url == URLS.SEND_REDPACK){//发红包接口
    params = this._extendWithDefault(params, [
      'mch_id',
      'nonce_str'
    ]);
  }else if(url == URLS.TRANSFERS){//企业转账接口
    params = this._extendWithDefault(params, [
      'nonce_str'
    ]);
  }else{
...
Payment.prototype.sendredpack = function(params, callback){
  this._signedQuery(URLS.SEND_REDPACK, params, {
    https: true,
    required: ['wxappid', 'mch_billno', 'send_name', 're_openid', 'total_amount', 'total_num', 'wishing', 'client_ip', 'act_name', 'remark']
  }, callback);
};
...
Payment.prototype.transfers = function(params, callback){
  this._signedQuery(URLS.TRANSFERS, params, {
    https: true,
    required: ['mch_appid', 'partner_trade_no', 'openid', 'check_name', 'amount', 'desc', 'spbill_create_ip']
  }, callback);
};
