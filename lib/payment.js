...
Payment.prototype.payBank = function(order, callback) {
  var self = this;
  var default_params = {
    mchid: this.mchId,
  };
  ...
  var requiredData = ['partner_trade_no', 'enc_bank_no', 'enc_true_name', 'bank_code', 'amount', 'desc'];

  this._signedQuery(URLS.PAY_BANK, order, {
    https: true,
    required: requiredData
  }, callback);
}
...
Payment.prototype._signedQuery = function(url, params, options, callback) {
  ...
  } else if (url == URLS.TRANSFERS || url == URLS.PAY_BANK) {
    params = this._extendWithDefault(params, [
      'nonce_str'
    ]);
  } else {
